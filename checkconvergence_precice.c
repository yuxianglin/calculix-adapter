...
void checkconvergence_precice(double *co, ITG *nk, ITG *kon, ITG *ipkon, char *lakon,
	...
	*theta=*theta+*dtheta;
	...
	/* check whether next increment size must be decreased */
		...
		*dthetaref=*dtheta;
		...
		printf("I am convergence\n\n");}
        FILE *cf;
        const char * temp_filename="~converge.dat";
        const char*  filename="convergence.dat";
        int converg=1;
        while (access(filename, F_OK) == 0)
            continue;

        if ((cf=fopen(temp_filename, "w"))!=NULL){
            fprintf(cf, "%d", converg);
            printf("I am writing the TRUE to file\n");
        } 
        fclose(cf);
	...
	if(((*iit>ic)&&(*mortar==0))||((*mortar>1)&&(*iit>200))){
        ...
        printf("\n *Trying to re-iteratate with half dt \n");
        FILE *cf;
        const char * temp_filename="~converge.dat";
        const char*  filename="convergence.dat";
        int converg=0;
        if ((cf=fopen(temp_filename, "w"))!=NULL){
            fprintf(cf, "%d", converg);
            printf("I am writing the FALSE to file\n");
        } 
        fclose(cf);
        rename(temp_filename, filename);

		*dtheta=*dthetaref*0.5;
		*dthetaref=*dtheta;
	...
	if((*iit>=i0)||(fabs(ram[0])>1.e20)||(fabs(cam[0])>1.e20)||
	   ...
	   ((cam[2]<1.e30)&&(cam[2]>*deltmx))||(idivergence==1)||
		...
		if(*idrct==1){
			...
			printf("\n *ERROR: solution seems to diverge;\n");
            printf("\n *Trying to re-iteratate with half dt \n");
            FILE *cf;
            const char * temp_filename="~converge.dat";
            const char*  filename="convergence.dat";
            int converg=0;
            while (access(filename, F_OK) == 0)//have convergence file
                continue;
            if ((cf=fopen(temp_filename, "w"))!=NULL){
                fprintf(cf, "%d", converg);
                printf("I am writing the FALSE to file\n");
